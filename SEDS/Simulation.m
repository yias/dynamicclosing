function [x xd t xT x_obs, sp]=Simulation(x0,xT,fn_handle,varargin)
%
...
if options.plot %plotting options
    ...
    if d>1
        sp = plot_results_nv('i',sp,x,xT,obs);
    else
        sp = plot_results_nv('i',sp,[x;0],[xT;0],obs);
    end
end
%% Simulation
i=1;
...
function sp = plot_results_nv(mode,sp,x,xT,varargin)
...
%                 figure(sp.fig)
            ...
            for i=2:d
                %sp{i-1}.axis(i-1)=subplot(d-1,1,i-1);
                figure(sp{i-1}.fig)
                hold on
                sp{i-1}.xT(i-1) = plot(xT(1),xT(i),'k*','markersize',10,'linewidth',1.5);
                sp{i-1}.xT_l(i-1) = plot(xT(1),xT(i),'k--','linewidth',1.5);
                for j=1:nbSPoint
                    plot(x(1,1,j),x(i,1,j),'ok','markersize',2,'linewidth',7.5);
                    sp{i-1}.x(i-1,j)= plot(x(1,1,j),x(i,1,j));
                end
                ylabel(['$\xi_' num2str(i) '$'],'interpreter','latex','fontsize',12);
                grid on
%                 if i==d
...
%         end
        ...
        elseif d==3
            ...
            for i=1:d-1
                figure(sp{i}.fig)
                ax=get(sp{i}.fig.CurrentAxes);
                for j=1:nbSPoint
                    set(sp{i}.x(i,j),'XData',x(1,:,j),'YData',x(i+1,:,j))
                end
                
                if max(x(1,end,:))>ax.XLim(2) || min(x(1,end,:))<ax.XLim(1) || max(x(i+1,end,:))>ax.YLim(2) || min(x(i+1,end,:))<ax.YLim(1)
%                     axis(sp{i}.axis(i),'tight');
%                     ax=get(sp{i}.axis(i));
                    axis(sp{i}.fig.CurrentAxes,'tight');
                    ax=get(sp{i}.fig.CurrentAxes);
                    axis(sp{i}.fig.CurrentAxes,...
                        [ax.XLim(1)-(ax.XLim(2)-ax.XLim(1))/10 ax.XLim(2)+(ax.XLim(2)-ax.XLim(1))/10 ...
                        ax.YLim(1)-(ax.YLim(2)-ax.YLim(1))/10 ax.YLim(2)+(ax.YLim(2)-ax.YLim(1))/10]);
                end
            end
