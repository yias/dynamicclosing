...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load('data/graspdata.mat'); %load 'data'
   data=graspdata';
...
%   plot(data(1,:), data(n+1,:), 'xr', 'markerSize', 5);
% %   plot(data(1,52:2*N), data(n+1,52:2*N), '-xg', 'markerSize', 5);
% %   plot(data(1,103:3*N), data(n+1,103:3*N), '-xb', 'markerSize', 5);
%   axis([min(data(1,:)) max(data(1,:)) min(data(n+1,:))-0.01 max(data(n+1,:))+0.01]);
%   xlabel('\mu','fontsize',16); ylabel('r','fontsize',16);
% end
% %plot 2D
...
for n=1:nbVar-1
  ...
  plotGMM(Mu([1,n+1],:), Sigma([1,n+1],[1,n+1],:), [0 0.5 0.5], 1);
  axis([min(data(1,:)) max(data(1,:))+0.1 0 300]);
  xlabel('Friction','fontsize',18); ylabel('r=torque/weight','fontsize',18);
  title('The torque/weight ratio decreases when friction increases','fontsize',24)
hold on
end
% %plot 2D
...
for n=1:nbVar-1
  ...
  plotGMM(expdata([1,n+1],:), expSigma(n,n,:), [0 0.5 0.5], 3);
  axis([min(data(1,:)) max(data(1,:)) 0 200]);
  xlabel('Friction','fontsize',18); ylabel('r=torque/weight','fontsize',18);
  grid on
end
