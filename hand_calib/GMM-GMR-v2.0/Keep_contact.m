...
%   pages="286--298",
% }
load('data/Keep_contact_data.mat');
data=[Keep_contact_data(2,:);Keep_contact_data(1,:)];
nbVar = size(data,1);
N=size(data,2)/3;
...
% xlabel('wg','fontsize',16); ylabel('F_m','fontsize',16);

%% Training of GMM by EM algorithm, initialized by k-means clustering.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[Priors, Mu, Sigma] = EM_init_kmeans(data, nbStates);
[Priors, Mu, Sigma, Pix,loglik] = EM(data, Priors, Mu, Sigma);
...
for n=1:nbVar-1
  ...
  plot(data(1,:), data(n+1,:), 'b*', 'markerSize', 2);
...
%   plot(data(1,103:3*N), data(n+1,103:3*N), '-xb', 'markerSize', 5);
  ...
  xlabel('Data point','fontsize',16); ylabel('Voltage','fontsize',16);
end
...
for n=1:nbVar-1
  subplot(2,1,1); hold on;
  title('Contact Detection','FontSize',24)
  plotGMM(Mu([1,n+1],:), Sigma([1,n+1],[1,n+1],:), [0 0 0], 1);
  axis([min(data(1,:)) max(data(1,:)) min(data(n+1,:))-0.01 max(data(n+1,:))+0.01]);
end
% %plot 2D
...
for n=1:nbVar-1
  ...
  plotGMM(expdata([1,n+1],:), expSigma(n,n,:), [0 0 0], 3);
  axis([min(data(1,:)) max(data(1,:)) min(data(n+1,:))-0.01 max(data(n+1,:))+0.01]);
  xlabel('Data point','fontsize',16); ylabel('Voltage','fontsize',16);
end
% %plot 2D
